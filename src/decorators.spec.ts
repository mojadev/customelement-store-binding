...
describe('decorators test', () => {
  ...
  describe('with scope store reference', () => {
    ...
    xit('should allow to switch the store scope during runtime', () => {
      const secondState = jasmine
        .createSpy('secondState')
        .and.returnValue({ test: 'state2' });
      const secondSubscribe = jasmine
        .createSpy('secondSubscribe')
        .and.returnValue({ test: 'subscribe2' });
